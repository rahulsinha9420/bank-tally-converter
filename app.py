...
def generate_tally_xml(df, output_path, conversion_type, main_ledger_name):
    ...
    for index, row in df.iterrows():
        ...
        if conversion_type == 'sales':
            ...
            is_party_debit = "Yes" if debit > 0 else "No"
            is_bank_credit = "No" if debit > 0 else "Yes"
...
def convert():
    try:
        ...
        if file:
            ...
            output_path = os.path.join(app.config['UPLOAD_FOLDER'], f"{os.path.splitext(filename)[0]}.xml")
            ...
            try:
                ...
                if df.empty: return jsonify({'error': "No data recognized"}), 400
            ...
            return send_file(output_path, as_attachment=True, download_name=f"{os.path.splitext(filename)[0]}.xml")
    except Exception as e:
